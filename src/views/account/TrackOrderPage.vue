      ...
      <nav class="navbar navbar-expand-lg sticky-top bg-white shadow-sm py-3">
        <div class="container">
          <a class="navbar-brand d-flex align-items-center" href="/">
            <img src="../../assets/Agroconnect.png" alt="AgroConnect Logo" height="40" class="me-2" />
          </a>
          <button
          ...
          <div class="card-body p-4">
            ...
            <div class="row mb-4">
              <div class="col-md-6">
                ...
                <p class="mb-2"><strong>Order Date:</strong> {{ orderInfo.date }}</p>
                <p class="mb-2"><strong>Total Amount:</strong> Rs {{ orderInfo.total.toLocaleString() }}</p>
                <p class="mb-0">
                  <strong>Status:</strong>
                  <span class="badge rounded-pill ms-2" :class="getStatusClass(orderInfo.status)">
                    {{ orderInfo.status }}
                  </span>
                </p>
              </div>
              <div class="col-md-6">
                <div class="d-md-flex justify-content-md-end h-100 align-items-md-center">
                  <div class="estimated-delivery p-3 bg-light rounded text-center">
            ...
            <div class="tracking-timeline my-5">
              ...
              <!-- Timeline progress -->
            <br>  <div class="position-relative mb-5">
                <!-- Progress bar and steps -->
                <div class="progress" style="height: 5px;">
                  <div class="progress-bar bg-success" :style="{ width: getProgressWidth() }"></div>
                </div>
                
                <!-- Timeline points -->
<div class="timeline-points d-flex justify-content-between position-absolute w-100" 
  ...
  <div v-for="(step, index) in trackingSteps" :key="index" 
    ...
    <div class="point-circle rounded-circle" 
         :class="{ 'active': isStepActive(index), 'completed': isStepCompleted(index) }">
      <i v-if="isStepCompleted(index)" class="bi bi-check"></i>
              ...
              <!-- Current status card -->
              <br><br><br><div class="current-status p-4 border rounded mb-4">
                <div class="d-flex align-items-center">
                  <div class="status-icon me-3 rounded-circle bg-light d-flex align-items-center justify-content-center">
                    <i :class="getCurrentStatusIcon()"></i>
                  </div>
                  <div>
                    <h4 class="h6 mb-1">{{ getCurrentStatusText() }}</h4>
                    <p class="mb-0 text-muted">{{ getCurrentStatusDescription() }}</p>
                  </div>
                </div>
              </div>
              ...
              <div class="tracking-history">
                ...
                <div class="tracking-events">
                  <div v-for="(event, index) in getTrackingHistory()" :key="index" 
                    ...
                    <div class="d-flex">
                      ...
                      <div>
                        ...
                        <p class="small text-muted mb-0">{{ event.date }} {{ event.time }}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="row mt-5">
              <div class="col-md-4 mb-4 mb-md-0">
                ...
                <p class="mb-1"><strong>{{ orderInfo.shipping.name }}</strong></p>
                <p class="mb-1">{{ orderInfo.shipping.address }}</p>
                <p class="mb-1">{{ orderInfo.shipping.city }}, {{ orderInfo.shipping.state }} {{ orderInfo.shipping.zip }}</p>
              ...
              <div class="col-md-4 mb-4 mb-md-0">
                ...
                <p v-if="orderInfo.shipping.trackingNumber" class="mb-0">
                  <strong>Tracking #:</strong> {{ orderInfo.shipping.trackingNumber }}
                </p>
              </div>
              ...
              <table class="table mb-0">
                ...
                <tbody>
                  <tr v-for="(item, index) in orderInfo.items" :key="index">
                    <td>
                      <div class="d-flex align-items-center">
                        <img :src="item.image" :alt="item.name" class="product-thumbnail me-3" width="60" height="60">
                        <span>{{ item.name }}</span>
                      </div>
                    </td>
                ...
                <tfoot class="table-light">
                  <tr>
                    ...
                    <td class="text-end">Rs {{ orderInfo.subtotal.toLocaleString() }}</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td class="text-end"><strong>Rs {{ orderInfo.total.toLocaleString() }}</strong></td>
                  </tr>
                </tfoot>
              </table>
            </div>
    ...
    methods: {
      ...
      getCurrentStatusDescription() {
        const statusDescriptions = {
          ...
          "Confirmed": "Your order has been confirmed and payment has been received.",
